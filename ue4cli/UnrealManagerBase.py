...
class UnrealManagerBase(object):
	...
	def validBuildStateConfigurations(self):
		"""
		Returns the list of valid build state configurations supported by UnrealBuildTool
		"""
		return ['Debug', 'DebugGame', 'Development', 'Shipping', 'Test']

	def validBuildTargetConfigurations(self):
		"""
		Returns the list of valid build target configurations supported by UnrealBuildTool
		"""
		return ['Editor', 'Client', 'Server']
	...
	def packageProject(self, dir=os.getcwd(), stateConfiguration='Shipping', targetConfiguration='Client', extraArgs=[]):
		"""
		Packages a build of the Unreal project in the specified directory, using common packaging options
		"""
		if targetConfiguration == 'Editor' and stateConfiguration in ('Shipping', 'Test'):
			raise UnrealManagerException('incompatible build configurations {0} {1}'.format(
				stateConfiguration, targetConfiguration))
		...
		if stateConfiguration == 'Shipping':
			extraArgs.append('-nodebuginfo')

		# If we are packaging a Server build, do not include client
		if targetConfiguration == 'Server':
			extraArgs.append('-server')
			extraArgs.append('-noclient')
		else:
		...
		self.runUAT([
			'BuildCookRun',
			'-utf8output',
			'-clientconfig=' + stateConfiguration,
			'-serverconfig=' + stateConfiguration,
	...
	def packageDescriptor(self, dir=os.getcwd(), args=[]):
		...
		# Default state and target
		stateConfiguration = 'Shipping'
		targetConfiguration = 'Client'

		if args and args[0] in self.validBuildStateConfigurations():
			stateConfiguration = args.pop(0)
			if args and args[0] in self.validBuildTargetConfigurations():
				targetConfiguration = args.pop(0)

		# Perform the packaging step
		if self.isProject(descriptor):
			self.packageProject(dir, stateConfiguration, targetConfiguration, args)
		else:
